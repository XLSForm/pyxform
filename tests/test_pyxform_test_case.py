...
class TestPyxformTestCaseXmlXpath(PyxformTestCase):
    ...
    suite2_counts = [c.ctuple for c in suite1]
    suite2_exacts = [c.etuple for c in suite1]
    suite2_xpaths = [c.xpath for c in suite1]
    ...
    @unittest.skip("Scenario '1_xpath_n_match' not applicable to xpath_match")
    ...
    def test_xml__n_xpath_n_match_pass__xpath_match(self):
        """Should pass for multiple XPaths with multiple matches each."""
        self.assertPyxformXform(
            md=self.md,
            xml__xpath_match=self.suite2_xpaths,
            run_odk_validate=False,
        )
    ...
    def test_xml__n_xpath_n_match_fail__xpath_match(self):
        """Should fail when matches not found for one XPath, among other passes."""
        with self.assertRaises(self.failureException):
            self.assertPyxformXform(
                md=self.md,
                xml__xpath_match=self.suite2_xpaths + [self.s3c1.xpath],
                run_odk_validate=False,
            )
